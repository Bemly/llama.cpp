...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv(ggml_metal_library_t lib, const ggml_tensor * op) {    GGML_TENSOR_LOCALS( int32_t, ne0, op->src[0], ne);
    ...
    switch (tsrc0) {
        ...
        case GGML_TYPE_PQ2_0:
            {
                nsg = ggml_metal_env_phase_int_(is_decode_like,
                                                "GGML_METAL_DECODE_PQ2_0_NSG",
                                                "GGML_METAL_PP_PQ2_0_NSG",
                                                "GGML_METAL_PQ2_0_NSG", N_SG_PQ2_0, 1, 8);
                nr0 = N_R0_PQ2_0;
            } break;
        case GGML_TYPE_Q4_0:
            {
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_flash_attn_ext_amd_tile(
    ...
    snprintf(base, 256, "kernel_flash_attn_ext_amd_tile_dk%d_nbc%d", dk, nbc);
    snprintf(name, 256, "%s_mask=%d", base, has_mask ? 1 : 0);
