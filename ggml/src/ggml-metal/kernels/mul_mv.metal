...
// PQ2_0 dot against coefficients staged by the caller (same codec as Q2_0,
// group 128 vs 64). A byte is a base-4 fraction of 256: with u = b/256 and
// g_k = floor(4^k*u), exact in fp32, the floor chain peels the fields from the
// top down. Summing t_n*y_n over the four fields collapses to
//   g_1*(y_3 - 4*y_2) + g_2*(y_2 - 4*y_1) + g_3*(y_1 - 4*y_0) + b*y_0
// so the caller stages those coefficients in place of the raw y and this is
// three floors and four fmas per byte with no integer work. sumy is subtracted
// once for the -1 offset.
...
// ── RX6800 phase variants (ported from Basten7/iRon-Llama-RC2 layer 2/5) ──
// PP likes larger nr0, decode stays at the small default; device.cpp picks the
// variant by phase via GGML_METAL_{PP,DECODE}_*_NR0 env knobs.
...
kernel void kernel_mul_mv_q4_K_f32_nr0_8(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
...
kernel void kernel_mul_mv_q6_K_f32_nr0_4(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
...
kernel void kernel_mul_mv_q6_K_f32_nr0_8(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
...
kernel void kernel_mul_mv_q8_0_f32_nr0_4(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
...
kernel void kernel_mul_mv_q4_0_f32_nr0_4(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
        ...
        ushort tiisg[[thread_index_in_simdgroup]],
        ushort sgitg[[simdgroup_index_in_threadgroup]]) {
    mul_vec_q_n_f32_impl<block_q4_0, 4, constant ggml_metal_kargs_mul_mv &>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg);
}

kernel void kernel_mul_mv_q5_0_f32_nr0_4(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
