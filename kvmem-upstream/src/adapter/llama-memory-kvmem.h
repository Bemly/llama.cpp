...
private:
    ...
    // Metal blit layout: gather resident rows to a private scratch buffer,
    // then scatter to orig_pos slots. Same byte moves as the CUDA D2D path.
    struct LayoutItem {
        uint32_t id;
        uint32_t orig;
        int32_t slot;
    ...
    bool layout_gpu_slots_metal(const LayoutItem * items, size_t n_items,
