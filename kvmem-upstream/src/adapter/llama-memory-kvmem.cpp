...
#include "llama-kvmem-cuda-shim.h"
#else
#include "ggml-cuda.h"
...
#if defined(LLAMA_KVMEM_METAL)
    // Discrete Metal has no CPU-visible device memory: raw pointers never
    // occur, so every caller takes its ggml-API fallback. Never return
    // t->data here (mirrors/staging are not device memory).
    (void) t;
    return nullptr;
#else
    if (!t || !t->data) {
...
#if defined(LLAMA_KVMEM_METAL)
    // Only host buffers ever reach here on Metal (device pointers are null).
...
#if defined(LLAMA_KVMEM_METAL)
bool llama_memory_kvmem::layout_gpu_slots_metal(const LayoutItem * items, size_t n_items,
                                                const size_t * res_ix, size_t n_res,
                                                size_t krow, size_t vrow) {
    if (!items || n_items == 0 || !res_ix || n_res == 0 || !kv_) {
        return false;
    }
    ...
    for (uint32_t il = 0; il < n_layer_; ++il) {
        ...
        ref = kv_->get_k_storage(static_cast<int32_t>(il));
        if (ref) {
            break;
        }
    }
    ...
    void * scratch = kvmem_metal_scratch_alloc_for(
        ref, static_cast<size_t>(n_res) * static_cast<size_t>(scratch_stride));
    if (!scratch) {
        return false;
    }
    bool ok = true;
    std::vector<kvmem_metal_move> gather;
    std::vector<kvmem_metal_move> scatter;
    gather.reserve(n_res * 2);
    scatter.reserve(n_res * 2);
    for (uint32_t il = 0; il < n_layer_ && ok; ++il) {
        ...
        const bool use_v = (vt != nullptr) && !v_trans_ && vrow != 0;
        gather.clear();
        scatter.clear();
        for (size_t i = 0; i < n_items; ++i) {
            ...
            const size_t soff = static_cast<size_t>(res_ix[i]) *
                                static_cast<size_t>(scratch_stride);
            if (krow && nt) {
                const size_t nb = static_cast<size_t>(nt) * krow;
                gather.push_back({kt, static_cast<size_t>(src0) * krow, soff, nb, 1});
                scatter.push_back({kt, static_cast<size_t>(dst0) * krow, soff, nb, 0});
            }
            if (use_v && nt) {
                const size_t nb = static_cast<size_t>(nt) * vrow;
                const size_t voff = soff + static_cast<size_t>(kspan);
                gather.push_back({vt, static_cast<size_t>(src0) * vrow, voff, nb, 1});
                scatter.push_back({vt, static_cast<size_t>(dst0) * vrow, voff, nb, 0});
            }
        }
        const int64_t t_g = ggml_time_us();
        if (!gather.empty() &&
            !kvmem_metal_blit_moves(gather.data(), static_cast<int>(gather.size()), scratch)) {
            ok = false;
            break;
        }
        if (retr_.enabled) {
        ...
        if (!scatter.empty() &&
            !kvmem_metal_blit_moves(scatter.data(), static_cast<int>(scatter.size()), scratch)) {
            ok = false;
            break;
        }
        if (retr_.enabled) {
    ...
    kvmem_metal_scratch_free_buf(scratch);
    if (!ok) {
        LLAMA_LOG_ERROR("%s: metal blit layout failed; host fallback\n", __func__);
    }
    return ok;
}
#endif
...
bool llama_memory_kvmem::layout_gpu_slots_by_orig_pos() {
...
#if defined(LLAMA_KVMEM_METAL)
    // Blit fast path (default on): same slot-to-slot byte moves as the CUDA
    // D2D block below, via Metal blit. MTP keeps the host path. On success
    // the CUDA block and the host round-trip are skipped, but the shared
    // tail below (seq_rm / realloc / occupy-or-rewrite / flush) still runs.
    // Any failure falls through to the host round-trip.
    bool metal_moved = !mtp_ && kvmem_metal_blit_enabled() &&
        layout_gpu_slots_metal(items.data(), items.size(), res_ix.data(), n_res,
                               krow, vrow);
    if (metal_moved && trace_) {
        fprintf(stderr, "KVMEM_TRACE layout_metal_blit n_res=%zu\n", n_res);
    }
#else
    const bool metal_moved = false;
#endif

    bool d2d_ok = n_res > 0;
#if defined(LLAMA_KVMEM_METAL)
    d2d_ok = d2d_ok && !metal_moved;
#endif
    ...
    for (size_t i = 0; i < items.size(); ++i) {
        ...
        store.set_block_tier(items[i].id, kvmem::KvTier::GPU, -1,
                             store.blocks()[items[i].id].nvme_slot);
        if (items[i].resident && (d2d_ok || metal_moved)) {
            occupy_block_cells(items[i].id);
...
void llama_memory_kvmem::harvest_gpu_v(uint32_t block_id) {
    ...
    kvmem_stagein_gpu_ready((size_t) block_tokens_ * std::max(n_embd_k_, n_embd_v_),
                            std::max(krow, vrow) * (size_t) block_tokens_);
#if defined(LLAMA_KVMEM_METAL)
    // Batched harvest D2H: defer per-row tensor_get, flush once per block
    // (one shared staging buffer, one blit, one wait, zero extra copies).
    struct MetalHJob {
        ggml_tensor * t;
        size_t toff;
        size_t nbytes;
    ...
    std::vector<MetalHJob> mh_jobs;
    size_t mh_total = 0;
    const bool mh_defer = kvmem_metal_blit_enabled();
#endif
...
#if defined(LLAMA_KVMEM_METAL)
        if (mh_defer && t && nbytes > 0) {
            mh_jobs.push_back({t, toff, nbytes, mh_total, nget, il, is_k});
            mh_total += nbytes;
            return;
        }
#endif
...
#if defined(LLAMA_KVMEM_METAL)
    if (!mh_jobs.empty()) {
        const int64_t t_h = retr_.enabled ? ggml_time_us() : 0;
        bool flushed = false;
        void * staging = kvmem_metal_staging_alloc_for(mh_jobs[0].t, mh_total);
        if (staging) {
            std::vector<kvmem_metal_move> moves;
            moves.reserve(mh_jobs.size());
            for (const auto & j : mh_jobs) {
                moves.push_back({j.t, j.toff, j.stage_off, j.nbytes, 1});
            }
            if (kvmem_metal_blit_moves(moves.data(), static_cast<int>(moves.size()), staging)) {
                const uint8_t * base =
                    static_cast<const uint8_t *>(kvmem_metal_staging_bytes(staging));
                if (base) {
                    for (const auto & j : mh_jobs) {
                        if (j.is_k) {
                            raw_->write_layer_k_gpu(blk.orig_pos_start, j.nget, j.il,
                                                    base + j.stage_off);
                        } else {
                            raw_->write_layer_v_gpu(blk.orig_pos_start, j.nget, j.il,
                                                    base + j.stage_off);
                        }
                    }
                    flushed = true;
            ...
            kvmem_metal_staging_free_buf(staging);
        }
        if (!flushed) {
            LLAMA_LOG_WARN("%s: metal harvest blit failed, per-row fallback\n", __func__);
            for (const auto & j : mh_jobs) {
                std::vector<uint8_t> packed(j.nbytes);
                kvmem_tensor_get(j.t, packed.data(), j.toff, j.nbytes);
                if (j.is_k) {
                    raw_->write_layer_k_gpu(blk.orig_pos_start, j.nget, j.il, packed.data());
                } else {
                    raw_->write_layer_v_gpu(blk.orig_pos_start, j.nget, j.il, packed.data());
                }
            }
        }
        if (retr_.enabled) {
            retr_.stage_out_gpu_us += ggml_time_us() - t_h;
        }
    }
#endif
    if (trace_) {
