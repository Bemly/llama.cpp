...
int main(int argc, char ** argv) {
    ...
    if (st.spec_mtp) {
        ...
        // Metal: MTP draft attention with F16 KV is broken (garbage); inherit
        // the target KV type unless --spec-kv-dtype was given (matches CLI).
        if (!st.spec_cache_given) {
            st.spec_cache_type = st.cache_type_k;
        }
